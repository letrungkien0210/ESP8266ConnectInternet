...
/////////////////////// Những dòng code ở trên phần này là phần cài đặt, các bạn hãy đọc thêm về angularjs để hiểu, cái này không nhảy cóc được nha!
	...
	$scope.changeRelay = function() {
		console.log(`Changed relay from ${$scope.relay_status} to ${!$scope.relay_status}`);

		const json = {
			relay: $scope.relay_status ? 1 : 0,
		}
	...
	// Khi nhận được lệnh RELAY_STATUS
	mySocket.on('RELAY_STATUS', function(json) {
		console.log('recv RELAY_STATUS', json);
		$scope.relay_status = json.data;
	})
